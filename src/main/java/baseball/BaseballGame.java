...
public class BaseballGame {
    ...
    public int countStrike(List<String> userInput) {
        ...
        for (int idx = 0; idx < userInput.size(); idx++) {
            String userNumber = userInput.get(idx);
            if (isSameNumber(userNumber) && isSamePosition(idx, userNumber)) {
                strikeCnt++;
            }
        }
        return strikeCnt;
    ...
    private boolean isSameNumber(String userBall) {
    ...
    public boolean isSamePosition(int idx, String userNumber) {
        return answer.get(userNumber) == idx;
    }
